...
In this chapter we will provide an example of multi-class classification, using the ["Palmer Penguins" dataset](https://github.com/mcnakhaee/palmerpenguins).
...
As you can see, the target variable is categorical, which makes this a multi-class classification task:
...
Investigating various features:
...
Let's examine the weight of a penguin, by island, to see if we can develop an intuition for the dataset:

```{python}
import plotly.express as px

px.scatter(df, x="body_mass_g", y="island", color="species",
  title="Body Mass by Penguin Species", height=350
)
```
...
pairplot(df, hue="species", height=1.5) # inches
```
...
Since we have multiple features, it may be helpful to scale them. Here we are using a standard scaling approach:
...
There are coefficients for each of the classes in our target variable (i.e. the features have different predictive power for different species).

Wrapping the coefficients in a `DataFrame` using corresponding column and index labels will help us sort and compare them:
...
Now it's time to evaluate the model against the test set. We are using our standard classification metrics, focusing on the classification report:
