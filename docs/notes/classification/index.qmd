...
When the target class has only two unique values, we perform ["binary classification" task](./binary.ipynb), whereas when the target class has more than two unique values, we perform ["multi-class classification" task](multiclass.qmd).
